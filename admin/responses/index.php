...
<body class="admin-body">
    ...
    <main class="main-content">
        <div class="responses-header">
            <div>
                <h1 class="page-title">📋 תשובות משתמשים</h1>
                <p class="page-subtitle">צפייה וניהול תשובות למילוי טפסים</p>
            </div>
        ...
        <div class="stats-cards" id="stats-cards">
            <div class="stat-card">
                ...
                <div class="stat-label">סך הכל תשובות</div>
            </div>
            <div class="stat-card">
                <div class="stat-value" id="stat-users">-</div>
                <div class="stat-label">משתמשים שענו</div>
            </div>
            <div class="stat-card">
        ...
        <div class="filters-section">
            <div class="filters-grid">
                <div class="filter-group">
                    <label class="filter-label">סינון לפי טופס</label>
                    <select id="filter-form" class="input">
                        <option value="">כל הטפסים</option>
                    </select>
                </div>
                <div class="filter-group">
                    <label class="filter-label">חיפוש משתמש</label>
                    <input type="text" id="filter-user" class="input" placeholder="חפש לפי תעודת זהות או שם">
                </div>
                <div class="filter-group">
                    <label class="filter-label">מצב השלמה</label>
                    <select id="filter-status" class="input">
                        <option value="">הכל</option>
                        <option value="complete">הושלם</option>
                        <option value="partial">חלקי</option>
                        <option value="none">לא התחיל</option>
                    </select>
                </div>
            </div>
                ...
                <tbody id="responses-tbody">
                    <tr>
                        <td colspan="7" style="text-align: center; padding: 2rem;">
                            ...
                            <p style="margin-top: 1rem; color: var(--gray-500);">טוען נתונים...</p>
                        </td>
                    </tr>
                </tbody>
            </table>
    ...
    <script>
            ...
            tbody.innerHTML = responses.map(response => {
                ...
                return `
                    <tr>
                        ...
                        <td>${escapeHtml(response.full_name || 'לא צוין')}</td>
                        ...
                        <td>
                            <div class="action-buttons">
                                <button class="btn btn-sm btn-primary" onclick="viewUserResponse(${response.user_id})" title="צפה בתשובות">
                                    👁️
                                </button>
                                <button class="btn btn-sm btn-secondary" onclick="exportUserResponse(${response.user_id})" title="ייצא CSV">
                                    📥 CSV
                                </button>
                                <button class="btn btn-sm btn-success" onclick="exportUserPDF(${response.user_id})" title="ייצא PDF">
                                    📄 PDF
                                </button>
                            </div>
                        </td>
        ...
        async function viewUserResponse(userId) {
            try {
                ...
                showError('שגיאה בטעינת תשובות משתמש');
            }
        }
        ...
        function showUserResponseModal(user, responses) {
            ...
            content.innerHTML = `
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 2rem;">
                    <h2 style="margin: 0;">תשובות: ${escapeHtml(user.full_name || user.tz)}</h2>
                    <button onclick="this.closest('.modal-overlay').remove()" class="btn btn-secondary">✖️</button>
                </div>
                <div style="margin-bottom: 1.5rem; padding: 1rem; background: var(--gray-50); border-radius: 8px;">
                    <p><strong>תעודת זהות:</strong> ${escapeHtml(user.tz)}</p>
                    <p><strong>שם מלא:</strong> ${escapeHtml(user.full_name || 'לא צוין')}</p>
                    <p><strong>סך הכל תשובות:</strong> ${responses.length}</p>
                </div>
                    ...
                    ${responses.length > 0 ? responses.map(r => `
                        <div style="padding: 1rem; border: 1px solid var(--gray-200); border-radius: 8px;">
                            ...
                            <div style="color: var(--gray-700); padding: 0.5rem; background: var(--gray-50); border-radius: 4px;">
                                ${escapeHtml(r.answer_value || r.answer_json || 'אין תשובה')}
                            </div>
                            <div style="font-size: 0.75rem; color: var(--gray-500); margin-top: 0.5rem;">
                                סוג שאלה: ${escapeHtml(r.type_name)} | תאריך: ${new Date(r.submitted_at).toLocaleString('he-IL')}
                            </div>
                        </div>
                    `).join('') : '<p style="text-align: center; color: var(--gray-500);">אין תשובות</p>'}
                </div>
            `;
        ...
        async function exportUserResponse(userId) {
            try {
                ...
                if (data.success) {
                    ...
                    downloadCSV(csv, `תשובות_${data.user.tz}.csv`);
        ...
        function exportToCSV() {
            let csv = '\uFEFF'; // BOM for Excel Hebrew support
            csv += 'תעודת זהות,שם מלא,טופס,תשובות שהושלמו,תאריך אחרון\n';
            ...
            downloadCSV(csv, 'תשובות_כל_המשתמשים.csv');
            showSuccess('✅ הקובץ יוצא בהצלחה');
        }
